...
# Local Imports
from AthenaTwitchBot.irc.logic import CommandLogic
...
class Bot:
    ...
    async def login(self):
        ...
        self.transport.write(twitch_output_format(f"PASS oauth:{self.oath_token}"))
        self.transport.write(twitch_output_format(f"NICK {self.name}"))
        for channel in self.join_channel:
            self.transport.write(twitch_output_format(f"JOIN #{channel}"))
        ...
        if self.capability_tags:  # this should always be requested, else answering to chat is not possible
            self.transport.write(twitch_output_format(f"CAP REQ :twitch.tv/tags"))
        if self.capability_commands:
            self.transport.write(twitch_output_format(f"CAP REQ :twitch.tv/commands"))
        if self.capability_membership:
            self.transport.write(twitch_output_format(f"CAP REQ :twitch.tv/membership"))

        # will catch all those that are Truthy (not: "", None, False, ...)
        if self.join_message:
            for channel in self.join_channel:
                self.transport.write(twitch_output_format(f"PRIVMSG #{channel} :{self.join_message}"))
